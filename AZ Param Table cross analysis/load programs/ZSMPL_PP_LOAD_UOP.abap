REPORT zsmpl_pp_load_uop.
*&---------------------------------------------------------------------*
*& ZSMPL_PP_LOAD_UOP  -  Load program for table ZTC_PP_UOP
*&   SiMPL Process Parameter - Unit Operation master
*& Package : ZSMPL_AZP2
*&
...
*& Excel "quoted" fields are supported, so values that contain a comma
*& (e.g. a description) survive intact. Expected header columns:
*&     UOP_ID   UOP_DESC   LONG_TEXT
*&
...
DATA: gt_raw  TYPE string_table,
      gt_data TYPE STANDARD TABLE OF ztc_pp_uop,
      gs_data TYPE ztc_pp_uop,
      gv_read TYPE i, gv_ins TYPE i, gv_upd TYPE i, gv_err TYPE i.

INITIALIZATION.
  gv_title = 'Load ZTC_PP_UOP (Unit Operation master)'.
...
FORM parse.
  ...
  LOOP AT lt_head INTO lv_val.
    ...
    ls_map-idx  = sy-tabix.
    INSERT ls_map INTO TABLE lt_map.        "duplicate headers ignored
  ENDLOOP.
  ...
  LOOP AT gt_raw INTO lv_line.
    ...
    ENDLOOP.
    IF gs_data-uop_id IS INITIAL. CONTINUE. ENDIF.   "skip key-less rows
    ...
    IF lv_q = abap_true.
      IF lv_c = '"'.
        IF lv_i + 1 < lv_len AND iv_line+lv_i(2) = '""'.
          CONCATENATE lv_fld '"' INTO lv_fld RESPECTING BLANKS. "escaped "
          lv_i = lv_i + 1.
        ELSE.
          lv_q = abap_false.                          "closing quote
        ENDIF.
      ELSE.
...
FORM post.
  IF p_del = abap_true AND p_test = abap_false.
    DELETE FROM ztc_pp_uop.
  ENDIF.
  LOOP AT gt_data INTO gs_data.
    SELECT SINGLE uop_id FROM ztc_pp_uop INTO @DATA(lv_x)
      WHERE uop_id = @gs_data-uop_id.
    IF sy-subrc = 0. ADD 1 TO gv_upd. ELSE. ADD 1 TO gv_ins. ENDIF.
    IF p_test = abap_false.
      MODIFY ztc_pp_uop FROM gs_data.
