REPORT zsmpl_pp_load_param.
*&---------------------------------------------------------------------*
*& ZSMPL_PP_LOAD_PARAM  -  Load program for table ZTC_PP_PARAM
*&   SiMPL Process Parameter - Parameter catalog
*& Package : ZSMPL_AZP2
*&
...
*& are ignored. Excel "quoted" fields are supported, so values with a
*& comma (e.g. FORMAT_EXAMPLE 'AZDXXXX, INT-XXX') survive. Expected:
*&   UOP_ID PARAM_ID PARAM_GROUP PARAM_ROLE PARAM_DESC DATA_TYPE UOM
*&   VALUE_TYPE IS_CALC FORMULA SRC_SYSTEM MAP_TYPE TOOL_READY IN_DTLS
*&   STEP_REF FORMAT_EXAMPLE
*& (MANDT and CREATED_/CHANGED_ fields are set by the program, not read.)
*&
*& NOTE: PARAM_ID is CHAR30 - values longer than 30 chars are truncated.
...
DATA: gt_raw  TYPE string_table,
      gt_data TYPE STANDARD TABLE OF ztc_pp_param,
      gs_data TYPE ztc_pp_param,
      gv_read TYPE i, gv_ins TYPE i, gv_upd TYPE i, gv_err TYPE i.

INITIALIZATION.
  gv_title = 'Load ZTC_PP_PARAM (Parameter catalog)'.
...
FORM post.
  IF p_del = abap_true AND p_test = abap_false.
    DELETE FROM ztc_pp_param.
  ENDIF.
  LOOP AT gt_data INTO gs_data.
    SELECT SINGLE created_by, created_on FROM ztc_pp_param
      INTO ( @gs_data-created_by, @gs_data-created_on )
      WHERE uop_id = @gs_data-uop_id AND param_id = @gs_data-param_id.
    IF sy-subrc = 0.
      ADD 1 TO gv_upd.
    ELSE.
    ...
    IF p_test = abap_false.
      MODIFY ztc_pp_param FROM gs_data.
